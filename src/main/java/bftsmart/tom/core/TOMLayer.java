...
public final class TOMLayer extends Thread implements RequestReceiver {
	...
	private final Lock replicaSpecificContentLock = new ReentrantLock(true);
	private final Condition replicaSpecificContentReceived = replicaSpecificContentLock.newCondition();
	...
	public TOMLayer(ExecutionManager manager,
        ...
        recoverer.setClientsManager(clientsManager);this.dt = new DeliveryThread(this, receiver, recoverer, this.controller); // Create delivery thread
		...
		if (readOnly) {
			...
			if (clientsManager.requestReceived(msg, fromClient, communication, responseManager)) {
				if (msg.hasReplicaSpecificContent()) {
					...
					replicaSpecificContentReceived.signalAll();
					replicaSpecificContentLock.unlock();
				}
				...
				for (TOMMessage request : requests) {
					...
					verifierExecutor.submit(() -> {
						try {
							...
							if (request.hasReplicaSpecificContent()) {
								do {
									replicaSpecificContent = clientsManager.getPrivateContentOf(request.getSender(),
											request.getSequence());
									if (replicaSpecificContent == null) {
										replicaSpecificContentLock.lock();
										replicaSpecificContentReceived.await();
										replicaSpecificContentLock.unlock();
									}
								} while (replicaSpecificContent == null);
							}
							...
							request.setReplicaSpecificContent(replicaSpecificContent);
	...
	public void processOutOfContext() {
		for (int nextConsensus = getLastExec() + 1;
			// DECISION_FORWARDING: process aut of context decisions
			 ...
			 nextConsensus = getLastExec() + 1) {// DECISION_FORWARDING: process out of context decisions if available
