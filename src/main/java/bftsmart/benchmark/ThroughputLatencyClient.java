		...
		public void run() {
			try {
				...
				if (initialClientId == clientId) {
					proxy.invokeOrdered(serializedWriteRequest, null, (byte)-1);
				}
				for (int i = 0; i < numOperations; i++) {
					...
					t1 = System.nanoTime();
					if (isWrite) {
						response = proxy.invokeOrdered(serializedWriteRequest, null, (byte)-1);
					} else {
						response = proxy.invokeUnordered(serializedReadRequest, null, (byte)-1);
					}
					t2 = System.nanoTime();
					latency = t2 - t1;
