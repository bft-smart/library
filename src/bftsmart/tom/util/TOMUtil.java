...
public class TOMUtil {
	...
	public static boolean verifySignature(Signature initializedSignatureEngine, byte[] message, byte[] signature)
		...
		/*
		 * logger.debug("Signature Algorithm: {}, " + "Provider: {}" ,
		 * initializedSignatureEngine.getAlgorithm(),
		 * initializedSignatureEngine.getProvider().getName());
		 */

		//System.out.println("\n\t\t TRYING VERIFY SIGNATURE.... LINE 232");
		/*
		 * try { Signature ecdsaVerify = Signature.getInstance("SHA512withRSA");
		 * ecdsaVerify.update(message); boolean resultSign =
		 * ecdsaVerify.verify(signature);
		 * logger.trace("ResultSign of VerifySignature: {} ", resultSign); } catch
		 * (SignatureException e) { logger.error("Failed to verify signature.");
		 * e.printStackTrace(); } catch (NoSuchAlgorithmException e) {
		 * logger.error("No Such Algoritm Exception."); e.printStackTrace(); }
		 */
