...
public class ServiceProxy extends TOMSender {
	...
	private CertificatedReply certificate = null; // Reply with partial signatures generated by replicas for the current request
		...
		extractor = (replyExtractor != null) ? replyExtractor : new Extractor() {
			...
			public TOMMessage extractResponse(TOMMessage[] replies, int sameContent, int lastReceived) {
				return replies[lastReceived];
                        ...
                        @Override
                        public SigShare[] extractSigShares(TOMMessage[] replies, int sameContent, int lastReceived) {
                            int count = 0;
                            for (int i = 0; i < replies.length; i++)
                                if (replies[i] != null &&
                                        replies[i].getSigShare() != null) count++;
                            
                            if (count < replyQuorum) return null;
                            
                            SigShare[] shares = new SigShare[count];

                            for (int i = 0, j = 0; i < replies.length; i++) {
                                if (replies[i] != null &&
                                        replies[i].getSigShare() != null) {
                                    shares[j] = replies[i].getSigShare();
                                    j++;
                                }
                            }
	...
	public int getInvokeUnorderedHashedTimeout() {
        ...
        public CertificatedReply getCertificatedReply() {
            return certificate;
        }
        
        public RSAPublicKey getPublicThresholdKey() {
            return getViewManager().getStaticConf().getRSAPublicThresholdKey();
        }
					...
					for (int i = 0; i < replies.length; i++) {
						...
						if ((i != pos || getViewManager().getCurrentViewN() == 1) && replies[i] != null
							...
							if (sameContent >= replyQuorum) {
								...
								SigShare[] shares = extractor.extractSigShares(replies, sameContent, pos);
                                                                if (shares != null) certificate = new CertificatedReply(response.getContent(),  getProcessId(), getSession(), reqId, shares);
                                                                reqId = -1;
								this.sm.release(); // resumes the thread that is executing the "invoke" method
								canReceiveLock.unlock();
								return;
